...
int16_t SX1278::setLnaBoost(bool enable) {
  int16_t state = SX127x::standby();
  if (enable) {
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_LNA, RADIOLIB_SX127X_LNA_BOOST_ON);
  } else {
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_LNA, RADIOLIB_SX127X_LNA_BOOST_OFF);
  }
  return (state);
}
