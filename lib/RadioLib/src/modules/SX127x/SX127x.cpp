...
int16_t SX127x::RxChainCalibration(void) {

    int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PA_CONFIG, 0x00); // Cut output
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::beginFSK(uint8_t chipVersion, float freqDev, float rxBw, uint16_t preambleLength, bool enableOOK) {
    ...
    state = SX127x::RxChainCalibration();
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::directMode() {
    ...
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                      RADIOLIB_SX127X_DIO1_CONT_DCLK | RADIOLIB_SX127X_DIO2_CONT_DATA, 5, 2);
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::startReceive(uint8_t len, uint8_t mode) {
    ...
    if (modem == RADIOLIB_SX127X_LORA) {
        ...
        if (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_HOP_PERIOD) > RADIOLIB_SX127X_HOP_PERIOD_OFF) {
            state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1, RADIOLIB_SX127X_DIO0_LORA_RX_DONE |
                                                                                 RADIOLIB_SX127X_DIO1_LORA_FHSS_CHANGE_CHANNEL,
                                              7, 4);
        } else {
            state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                              RADIOLIB_SX127X_DIO0_LORA_RX_DONE | RADIOLIB_SX127X_DIO1_LORA_RX_TIMEOUT,
                                              7, 4);
        }
        ...
        // set FIFO pointers
        state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_RX_BASE_ADDR,
                                           RADIOLIB_SX127X_FIFO_RX_BASE_ADDR_MAX);
        state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_ADDR_PTR, RADIOLIB_SX127X_FIFO_RX_BASE_ADDR_MAX);
        RADIOLIB_ASSERT(state);

    } else if (modem == RADIOLIB_SX127X_FSK_OOK) {
        // set DIO pin mapping
        state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1, RADIOLIB_SX127X_DIO0_PACK_PAYLOAD_READY, 7,
                                          6);
        RADIOLIB_ASSERT(state);
...
int16_t SX127x::startTransmit(uint8_t *data, size_t len, uint8_t addr) {
    ...
    if (modem == RADIOLIB_SX127X_LORA) {
        ...
        // set FIFO pointers
        state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_FIFO_TX_BASE_ADDR,
                                           RADIOLIB_SX127X_FIFO_TX_BASE_ADDR_MAX);
        ...
        if ((filter == RADIOLIB_SX127X_ADDRESS_FILTERING_NODE) ||
            (filter == RADIOLIB_SX127X_ADDRESS_FILTERING_NODE_BROADCAST)) {
...
int16_t SX127x::readData(uint8_t *data, size_t len) {
    ...
    if (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_IRQ_FLAGS, 5, 5) ==
    ...
    if (modem == RADIOLIB_SX127X_LORA) {
        ...
        if (this->crcEnabled && (state == RADIOLIB_ERR_NONE) &&
            (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_HOP_CHANNEL, 6, 6) == 0)) {
...
int16_t SX127x::startChannelScan() {
    ...
    // set DIO pin mapping
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_1,
                                      RADIOLIB_SX127X_DIO0_LORA_CAD_DONE | RADIOLIB_SX127X_DIO1_LORA_CAD_DETECTED, 7,
                                      4);
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::setAFC(bool isEnabled) {
    ...
    //set AFC auto on/off
    return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_RX_CONFIG,
                                      isEnabled ? RADIOLIB_SX127X_AFC_AUTO_ON : RADIOLIB_SX127X_AFC_AUTO_OFF, 4, 4));
}
...
int16_t SX127x::setNodeAddress(uint8_t nodeAddr) {
    ...
    int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                              RADIOLIB_SX127X_ADDRESS_FILTERING_NODE, 2, 1);
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::setBroadcastAddress(uint8_t broadAddr) {
    ...
    int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                              RADIOLIB_SX127X_ADDRESS_FILTERING_NODE_BROADCAST, 2, 1);
    RADIOLIB_ASSERT(state);
...
int16_t SX127x::disableAddressFiltering() {
    ...
    // disable address filtering
    int16_t state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                              RADIOLIB_SX127X_ADDRESS_FILTERING_OFF, 2, 1);
    RADIOLIB_ASSERT(state);
...
uint32_t SX127x::getTimeOnAir(size_t len) {
    ...
    if (modem == RADIOLIB_SX127X_LORA) {
        ...
        float n_pre = (float) ((this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_MSB) << 8) |
                               this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_LSB));
        // Get number of bits payload
        float n_pay = 8.0 + RADIOLIB_MAX(
                ceil((8.0 * (float) len - 4.0 * (float) this->spreadingFactor + 28.0 + 16.0 * crc - 20.0 * ih) /
                     (4.0 * (float) this->spreadingFactor - 8.0 * de)) * (float) this->codingRate, 0.0);

        // Get time-on-air in us
        return ceil(symbolLength * (n_pre + n_pay + 4.25)) * 1000;
    } else if (modem == RADIOLIB_SX127X_FSK_OOK) {
        // Get number of bits preamble
        float n_pre = (float) ((this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_MSB_FSK) << 8) |
                               this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PREAMBLE_LSB_FSK)) * 8;
        ...
        //Get CRC bits
        float crc =
                (this->mod->SPIgetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, 4, 4) == RADIOLIB_SX127X_CRC_ON) * 16;
...
int16_t SX127x::setEncoding(uint8_t encoding) {
    ...
    switch (encoding) {
        ...
        case RADIOLIB_ENCODING_MANCHESTER:
            return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, RADIOLIB_SX127X_DC_FREE_MANCHESTER,
                                              6, 5));
        case RADIOLIB_ENCODING_WHITENING:
            return (this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1, RADIOLIB_SX127X_DC_FREE_WHITENING, 6,
                                              5));
        default:
...
int16_t SX127x::configFSK() {
    ...
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_1,
                                      RADIOLIB_SX127X_PACKET_VARIABLE | RADIOLIB_SX127X_DC_FREE_NONE |
                                      RADIOLIB_SX127X_CRC_ON | RADIOLIB_SX127X_CRC_AUTOCLEAR_ON |
                                      RADIOLIB_SX127X_ADDRESS_FILTERING_OFF | RADIOLIB_SX127X_CRC_WHITENING_TYPE_CCITT,
                                      7, 0);
    state |= this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_PACKET_CONFIG_2,
                                       RADIOLIB_SX127X_DATA_MODE_PACKET | RADIOLIB_SX127X_IO_HOME_OFF, 6, 5);
    RADIOLIB_ASSERT(state);

    // set preamble polarity
    state = this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_SYNC_CONFIG, RADIOLIB_SX127X_PREAMBLE_POLARITY_AA, 5,
                                      5); // orig 55
    RADIOLIB_ASSERT(state);
    ...
    while ((i < 10) && !flagFound) {
        ...
        if (version == ver) {
            ...
            RADIOLIB_DEBUG_PRINTLN(
                    "SX127x not found! (%d of 10 tries) RADIOLIB_SX127X_REG_VERSION == 0x%04X, expected 0x00%X", i + 1,
                    version, ver);
            this->mod->hal->delay(10);
            i++;
        }
...
void SX127x::clearFHSSInt(void) {
    int16_t modem = getActiveModem();
    if (modem == RADIOLIB_SX127X_LORA) {
        this->mod->SPIwriteRegister(RADIOLIB_SX127X_REG_IRQ_FLAGS,
                                    getIRQFlags() | RADIOLIB_SX127X_CLEAR_IRQ_FLAG_FHSS_CHANGE_CHANNEL);
    } else if (modem == RADIOLIB_SX127X_FSK_OOK) {
...
int16_t SX127x::setDIOPreambleDetect(bool usePreambleDetect) {
    return this->mod->SPIsetRegValue(RADIOLIB_SX127X_REG_DIO_MAPPING_2,
                                     (usePreambleDetect) ? RADIOLIB_SX127X_DIO_MAP_PREAMBLE_DETECT
                                                         : RADIOLIB_SX127X_DIO_MAP_RSSI, 0, 0);
}
