...
void appendDataLog(SD_LOG sd, PagerClient::pocsag_data *p, const struct lbj_data& l, const struct rx_info& r){
    ...
    switch (l.type) {
        case 0:
        {
            if (l.direction == FUNCTION_UP)
                ...
                sd.append("[LBJ] 方向: %4d  ",l.direction);
    ...
    if (l.type != 1) {
        sd.append("[R:%3.1f dBm/F:%5.2f Hz]", r.rssi, r.fer);
        for (size_t i = 0; i < POCDAT_SIZE; i++) {
            if (p[i].is_empty)
                continue;
            sd.append("[%d/%d:%s]", p[i].addr, p[i].func, p[i].str.c_str());
            sd.append("[E:%02d/%02d/%zu]", p[i].errs_uncorrected, p[i].errs_total, (p[i].len / 5) * 32);
        }
        sd.append("\n");
    }
}
